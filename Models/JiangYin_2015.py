...
class JiangYin(Model):

    def custom_model1(self,inp, n_classes):
    ...
    def custom_model2(self, inp, n_classes):
        #Adapted architecture
        H = Conv2D(filters=5, kernel_size=(5, 5))(inp)
        H = Activation('relu')(H)
        H = AveragePooling2D(pool_size=(2, 2))(H)

        H = Conv2D(filters=10, kernel_size=(4, 4))(H)
        H = Activation('relu')(H)
        H = AveragePooling2D(pool_size=(2, 2))(H)
    ...
    def check_neighboring(self, sis, elem1, elem2):
        if(len(sis)==1):
            return False
    ...
    def activity_image(self, raw_signals):
        ...
        while i!=j:
            if j==len(seq):
                j=0
            elif self.check_neighboring(sis, i, j) == False:
                sis.append(j)
                i = j
                j = j+1
            else:
